...
mart_player_income.sql - Оценка призовых игроков (с допущением равного деления командного выигрыша)
...
# Запросы для дашбордов (Dashboard Queries)
...
1. Выполнить запрос в PostgreSQL (схема dm/dds)
2. Экспортировать результат в CSV
3. Загрузить CSV в Yandex DataLens
4. Построить график

# Папка /sql/marts/
Содержит аналитические витрины — SQL-запросы с готовыми агрегатами, которые отвечают на ключевые бизнес-вопросы. Каждая витрина может использоваться в разных дашбордах.

# Папка /sql/dashboard_queries/
Содержит специализированные запросы под конкретные визуализации в Yandex DataLens. Каждый файл соответствует одному графику на дашборде.
